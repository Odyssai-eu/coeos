...
import { createCoworkPipeline } from "@/pages/session/cowork/pipeline"
import { CoworkGateDock } from "@/pages/session/composer/cowork-gate-dock"
import { COWORK_STAGES } from "@/pages/session/cowork/stages"
import {
...
import { SessionCanvasPanel } from "@/pages/session/session-canvas-panel"
...
export default function Page() {
  ...
  // Review (diffs) n'a de sens qu'avec git. Sans repo git (cas Némo) : pas d'onglet
  // Review, pas de prompt "Create a Git repository", pas de vol d'activation.
  const canReview = createMemo(() => sync().project?.vcs === "git")
  ...
  // Mode Cowork — pipeline gaté composer→reviewer→validator (doc/COWORK-MODE.md).
  // Le gate est CODE-enforced : on attend busy→idle entre étages et on s'arrête.
  const cowork = createCoworkPipeline({
    busy,
    ...
    abort: (sessionID) => void sdk().client.session.abort({ sessionID }).catch(() => {}),
  })

  // Cowork : ouvrir cowork-doc.md dans le VIEWER standard (rendu md via file-tabs)
  // quand un tour cowork se termine — remplace l'ancien panneau bespoke.
  const openCoworkTab = createOpenSessionFileTab({
    normalizeTab,
    openTab: tabs().open,
    pathFromTab: file.pathFromTab,
    loadFile: file.load,
  ...
  let coworkSawBusy = false
  createEffect(() => {
    const id = params.id
    if (!id) return
    const isCowork = cowork.active() || local.agent.current()?.name === "cowork"
    if (!isCowork) {
      coworkSawBusy = false
      return
    }
    ...
    if (!coworkSawBusy) return
    coworkSawBusy = false
    openCoworkTab(file.tab("cowork-doc.md"))
  })

  // Explorer Files (fichiers du dossier projet local) : onglet "all" (pas
  // "changes" qui dépend de git) + ouvert par défaut sur desktop. Effet one-shot
  // (attend que isDesktop soit résolu — onMount ne le garantit pas).
  let didOpenFileTree = false
  createEffect(() => {
    if (didOpenFileTree || !isDesktop()) return
    didOpenFileTree = true
    settings.general.setShowFileTree(true) // force (override l'état persisté false)
    ...
    return (
      <SessionComposerRegion
        ...
        gate={
          cowork.awaitingGate() ? (
            <CoworkGateDock
              stageLabel={cowork.stageLabel()}
              nextLabel={cowork.nextStage() ? COWORK_STAGES[cowork.nextStage()!].label : undefined}
              nextLens={cowork.nextStage() ? COWORK_STAGES[cowork.nextStage()!].lens : undefined}
              isLast={cowork.isLast()}
              onNext={cowork.next}
              onStop={cowork.stop}
            />
          ) : undefined
    ...
    <SessionRouteFrame>
      ...
      >
        ...
        <SessionSidePanel
          ...
          size={size}
        />
        <Show when={settings.general.showSessionCanvas() && !!params.id && isDesktop()}>
          <SessionCanvasPanel sessionID={params.id!} />
        </Show>
      </div>
