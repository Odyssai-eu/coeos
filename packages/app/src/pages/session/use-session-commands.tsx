...
export const useSessionCommands = (actions: SessionCommandContext) => {
  ...
  // Export de la session en Markdown (transcription : role + texte de chaque tour).
  const exportSessionMd = () => {
    ...
    for (const m of messages) {
      ...
      const text = parts
        ...
        .trim()
      if (!text) continue
      blocks.push(`## ${m.role === "user" ? "Utilisateur" : "Assistant"}\n\n${text}`)
    }
    if (!blocks.length) {
      showToast({ variant: "error", title: "Rien à exporter" })
      return
    }
    const title = info()?.title || "session"
    saveTextAsFile(mdFilename(title), `# ${title}\n\n${blocks.join("\n\n---\n\n")}\n`)
  }

  // Export de la dernière réponse de l'assistant en Markdown.
  const exportLastResponseMd = () => {
    const id = params.id
    if (!id) return
    const messages = sync().data.message[id] ?? []
    const last = [...messages].reverse().find((m) => m.role === "assistant")
    if (!last) {
      showToast({ variant: "error", title: "Aucune réponse" })
      return
    }
    const text = (sync().data.part[last.id] ?? [])
      ...
      .trim()
    if (!text) {
      showToast({ variant: "error", title: "Réponse vide" })
      return
    }
    saveTextAsFile(mdFilename("reponse"), `${text}\n`)
  }
  ...
  const sessionCmds = () => [
    ...
    sessionCommand({
      ...
      title: "Exporter la session (.md)",
      slash: "export",
      disabled: !params.id || visibleUserMessages().length === 0,
      onSelect: exportSessionMd,
    }),
    sessionCommand({
      id: "session.export.response.md",
      title: "Exporter la dernière réponse (.md)",
      slash: "export-response",
      disabled: !params.id || visibleUserMessages().length === 0,
      onSelect: exportLastResponseMd,
    }),
  ]
