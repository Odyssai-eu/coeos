...
import { CoworkStageBanner } from "@/pages/session/cowork/cowork-stage-banner"
import { coworkStageOf } from "@/pages/session/cowork/stages"
...
export function MessageTimeline(props: {
  ...
  const renderAssistantPartGroup = (row: Accessor<TimelineRowMap["AssistantPart"]>, onSizeChange?: () => void) => {
    ...
    // Cowork (#47) : attribuer le tour à son étage. Le bandeau ne s'affiche que
    // sur la PREMIÈRE partie rendue du message — sinon il se répéterait à chaque
    // outil et chaque bloc de texte du même tour.
    const coworkStage = createMemo(() => {
      const msg = message()
      if (!msg || msg.role !== "assistant") return
      const stage = coworkStageOf((msg as AssistantMessage).agent)
      if (!stage) return
      const first = getMsgParts(msg.id).find((item) =>
        renderable(item, settings.general.showReasoningSummaries()),
      )
      return first && first.id === part()?.id ? stage : undefined
    })
      ...
      <Show when={message()}>
        {(message) => (
          <Show when={part()}>
            {(part) => (
              <>
                <Show when={coworkStage()}>{(stage) => <CoworkStageBanner stage={stage()} />}</Show>
                <MessagePart
                  part={part()}
                  message={message()}
