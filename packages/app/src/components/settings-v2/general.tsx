...
export const SettingsGeneralV2: Component<{
  ...
  // coeos-code — mode YOLO (accepte toutes les permissions sans demander).
  // Desktop only, restart-to-apply (le serveur lit le flag au spawn).
  const [yolo, { mutate: setYoloLocal }] = createResource(
    () => (desktop() && platform.getYolo ? true : false),
    () => Promise.resolve(platform.getYolo?.() ?? false).catch(() => false),
    { initialValue: false },
  )
  ...
  const onYoloChange = (checked: boolean) => {
    setYoloLocal(checked)
    const update = platform.setYolo?.(checked)
    if (update) void Promise.resolve(update).catch(() => setYoloLocal(!checked))
    showToast({
      variant: checked ? "error" : "default",
      title: checked ? "YOLO activé — aucune permission ne sera demandée" : "YOLO désactivé",
  ...
  const GeneralSection = () => (
    <div class="settings-v2-section">
      <SettingsListV2>
        {/* coeos-code: English-only — sélecteur de langue retiré. */}
        <SettingsRowV2
        ...
        <SettingsRowV2
          title={language.t("settings.general.row.sessionCanvas.title")}
          description={language.t("settings.general.row.sessionCanvas.description")}
        >
          <div data-action="settings-session-canvas">
            <Switch
              checked={settings.general.showSessionCanvas()}
              onChange={(checked) => settings.general.setShowSessionCanvas(checked)}
            />
          </div>
        </SettingsRowV2>
    ...
    <Show when={desktop()}>
      <div class="settings-v2-section">
        ...
        <SettingsListV2>
          ...
          <Show when={desktop() && platform.getYolo}>
            <SettingsRowV2
              title="YOLO mode"
              description="Grants every permission without asking, including destructive operations (rm -rf, push --force, reset --hard). No confirmation step. Restart required."
            >
              <div data-action="settings-yolo">
                <Switch checked={yolo.latest} onChange={onYoloChange} />
              </div>
            </SettingsRowV2>
          </Show>
        </SettingsListV2>
