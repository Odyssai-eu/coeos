...
type PromptSubmitInput = {
  ...
  onCowork?: (draft: FollowupDraft) => void
  onAbort?: () => void
  onSubmit?: () => void
}
...
export function createPromptSubmit(input: PromptSubmitInput) {
  ...
  const handleSubmit = async (event: Event) => {
    ...
    // Mode Cowork : l'agent sentinelle "cowork" n'est jamais envoyé — on lance
    // le pipeline gaté (composer→reviewer→validator) à la place. Voir
    // pages/session/cowork/pipeline.ts.
    if (mode === "normal" && agent === "cowork" && input.onCowork) {
      // Fire AUSSI sur session neuve : le session est déjà créé + navigué ci-dessus
      // (draft.sessionID valide), donc le pipeline renderer peut démarrer.
      input.onCowork(draft)
      clearContext(submission.target())
      clearInput()
      return
    }
