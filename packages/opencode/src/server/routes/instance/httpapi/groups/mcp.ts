...
// coeos-code v2 (Settings MCP, 2026-08-02) : `add` ci-dessus ne persiste qu'en
// mémoire (InstanceState) — les serveurs disparaissent au restart. `save`
// écrit dans le fichier de config UTILISATEUR (~/.config/opencode/opencode.json)
// PUIS connecte à chaud ; `remove` déconnecte + retire du fichier.
export const SavePayload = Schema.Struct({ name: Schema.String, config: ConfigMCPV1.Info })
export const RemovePayload = Schema.Struct({ name: Schema.String })
  ...
  .add(
    HttpApiGroup.make("mcp")
      .add(
        ...
        HttpApiEndpoint.post("save", McpPaths.save, {
          query: WorkspaceRoutingQuery,
          payload: SavePayload,
          success: described(StatusMap, "MCP server saved and connected"),
          error: HttpApiError.BadRequest,
        }).annotateMerge(
          OpenApi.annotations({
            identifier: "mcp.save",
            summary: "Save MCP server",
            description: "Persist an MCP server to the user config (~/.config/opencode/opencode.json) and connect it live.",
          }),
        ),
        HttpApiEndpoint.post("remove", McpPaths.remove, {
          query: WorkspaceRoutingQuery,
          payload: RemovePayload,
          success: described(Schema.Boolean, "MCP server removed"),
          error: HttpApiError.BadRequest,
        }).annotateMerge(
          OpenApi.annotations({
            identifier: "mcp.remove",
            summary: "Remove MCP server",
            description: "Disconnect an MCP server and remove it from the user config.",
          }),
        ),
