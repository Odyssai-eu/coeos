...
function promptErrorMessage(error: AssistantError) {
  ...
  return "coeos-code prompt failed"
}
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "coeos-code service failure", service })
}
