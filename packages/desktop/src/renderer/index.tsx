...
function windowLastActiveUrlKey(windowID: string) {
  return `codeos.desktop.window.${windowID}.last-active-url`
}
...
const createPlatform = (windowState: DesktopWindowState): Platform => {
  ...
  return {
    ...
    getYolo: () => window.api.getYolo(),

    setYolo: (enabled: boolean) => window.api.setYolo(enabled),

    getLocalEngine: () => window.api.getLocalEngine(),
    setLocalEngine: (url: string, token: string) => window.api.setLocalEngine(url, token),
    getExtraProviders: () => window.api.getExtraProviders(),
    setExtraProviders: (raw: string) => window.api.setExtraProviders(raw),
    getTriageModel: () => window.api.getTriageModel(),
    setTriageModel: (id: string) => window.api.setTriageModel(id),
    listDeclaredModels: () => window.api.listDeclaredModels(),
    getRouterName: () => window.api.getRouterName(),
    setRouterName: (name: string) => window.api.setRouterName(name),
    getResolvedEngine: () => window.api.getResolvedEngine(),
    getEngines: () => window.api.getEngines(),
    setEngines: (list) => window.api.setEngines(list),
    testEngine: (baseURL: string, apiKey: string) => window.api.testEngine(baseURL, apiKey),
    getPlanModel: () => window.api.getPlanModel(),

    setPlanModel: (modelID: string) => window.api.setPlanModel(modelID),

    getBuildModel: () => window.api.getBuildModel(),

    setBuildModel: (modelID: string) => window.api.setBuildModel(modelID),

    importMemoryVault: (sourcePath: string) => window.api.importMemoryVault(sourcePath),

    getMemoryVaultPath: () => window.api.getMemoryVaultPath(),

    getMemoryEmbedConfig: () => window.api.getMemoryEmbedConfig(),

    setMemoryEmbedConfig: (cfg: { embedURL?: string; embedModel?: string; embedKey?: string }) =>
      window.api.setMemoryEmbedConfig(cfg),
