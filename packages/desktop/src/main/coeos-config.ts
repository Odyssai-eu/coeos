...
import { DEFAULT_ROUTER_NAME, getEngines, getRouterName, type Engine } from "./coeos-engines"
...
import { BUILD_MODEL_KEY, ENGINE_CATALOG_KEY, MODEL_CATALOG_KEY, PLAN_MODEL_KEY, ROLE_ASSIGNMENT_KEY } from "./store-keys"
...
export const COEOS_DEFAULT_CONFIG = {
  ...
  provider: {
    coeos: {
      ...
      // Le routeur est la CoeOS box (LAN, api.coeos.io ou un VPS) — plus
      // « OdyssAI-x », qui désigne l'engine d'inférence, un autre moteur
      // (Sophie 2026-09-19 : « il y a un mélange entre odyssai-x et coeos »).
      // Libellé surchargeable par l'utilisatrice (ROUTER_NAME_KEY) ; l'id
      // `coeos` ne change jamais (triage, rôles, plugins en dépendent).
...
/** Bouton « Test » de Settings > Providers : joint le moteur MAINTENANT et
 * rend ce qu'il publie. Côté main (curl), jamais depuis le renderer : le
 * renderer ne voit pas le LAN de la même façon (multi-homed, CORS). Le
 * well-known est un bonus (vendor / kind) : un serveur OpenAI-compatible
 * quelconque n'en a pas et reste valide. */
export async function testEngine(
  ...
  if (!/^https?:\/\/\S+$/i.test(root)) return { ok: false, models: [], error: "URL invalide (http:// ou https://)." }
  const engine: ResolvedEngine = { baseUrl: root, apiKey: (apiKey || "").trim() || "dummy", source: "local" }
  const [models, meta] = await Promise.all([
    fetchServableModels(engine, budgetMs),
    curlJson(`${root}/.well-known/inference-engine.json`, Math.min(budgetMs, 3000)) as Promise<{ vendor?: string } | null>,
  ])
  const vendor = typeof meta?.vendor === "string" ? meta.vendor : undefined
  if (!models.length) {
    return {
      ok: false,
      models: [],
      vendor,
      error: "Aucun modèle publié : moteur injoignable, clé refusée, ou /v1/models vide.",
    }
  }
  return { ok: true, models: models.map((m) => m.id), vendor }
}
...
/** Catalogue d'un moteur SUPPLÉMENTAIRE (OdyssAI-x, autre box, serveur
 * OpenAI-compatible) : TOUT ce que /v1/models publie, enrichi (nom lisible,
 * fenêtre, tools). Contrairement au routeur (un seul modèle curaté, le
 * décideur choisit), un moteur déclaré expose ses modèles tels quels — c'est
 * ce qu'on vient y chercher. Le routeur CoeOS d'une seconde box (kind=router)
 * est gardé lui aussi : c'est un modèle servable comme un autre. Pure. */
export function engineModelsFromCatalog(
  models: ServableModel[],
): Record<string, { name: string; limit: { context: number; output: number }; tool_call?: boolean }> {
  const out: Record<string, { name: string; limit: { context: number; output: number }; tool_call?: boolean }> = {}
...
/** Bloc provider opencode d'un moteur déclaré. Pure. */
export function engineProviderBlock(engine: Engine, models: ServableModel[]) {
  return {
    npm: "@ai-sdk/openai-compatible",
    name: engine.name,
    options: { baseURL: `${engine.baseURL}/v1`, apiKey: engine.apiKey || "dummy" },
    models: engineModelsFromCatalog(models),
  }
}
...
export async function buildCoeosConfig(assets: CodeosAssets, engine?: ResolvedEngine | null) {
  ...
  // Les moteurs déclarés sont découverts EN PARALLÈLE du routeur : un moteur
  // éteint coûte au plus le budget d'un fetch, pas N fois.
  const engines = getEngines()
  const enginesFetch = Promise.all(
    engines.map((e) =>
      fetchServableModels({ baseUrl: e.baseURL, apiKey: e.apiKey || "dummy", source: "local" }).catch(
        () => [] as ServableModel[],
      ),
    ),
  )
  ...
  const baseProvider = engine
    ...
    : { ...COEOS_DEFAULT_CONFIG.provider, coeos: { ...COEOS_DEFAULT_CONFIG.provider.coeos, name: routerName } }
  // Moteurs déclarés : catalogue live, repli sur le dernier connu PAR moteur
  // (ENGINE_CATALOG_KEY). Un moteur sans aucun modèle (jamais joint, pas de
  // cache) n'est pas déclaré : un provider vide n'apporte rien au picker et
  // masquerait la cause — le bouton Test de Settings dit pourquoi.
  const engineCatalogs = await enginesFetch
  const cachedRaw = getStore().get(ENGINE_CATALOG_KEY)
  const cached: Record<string, ServableModel[]> =
    cachedRaw && typeof cachedRaw === "object" ? (cachedRaw as Record<string, ServableModel[]>) : {}
  const nextCache: Record<string, ServableModel[]> = {}
  const engineProviders: Record<string, ReturnType<typeof engineProviderBlock>> = {}
  engines.forEach((e, i) => {
    const live = engineCatalogs[i] ?? []
    const models = live.length ? live : Array.isArray(cached[e.id]) ? cached[e.id] : []
    if (models.length) nextCache[e.id] = models
    if (!models.length) return
    if (e.id === ROUTER_PROVIDER) return
    engineProviders[e.id] = engineProviderBlock(e, models)
  })
  if (engines.length) getStore().set(ENGINE_CATALOG_KEY, nextCache)
  else getStore().delete(ENGINE_CATALOG_KEY as never)
  ...
  const provider = { ...baseProvider, ...engineProviders, ...extraProvidersOf(getExtraProviders()) }
  return {
    ...COEOS_DEFAULT_CONFIG,
    provider,
    // Tableau REMPLACÉ au merge (pas fusionné) : un opencode.json global ou
    // projet qui porte `disabled_providers:["coeos"]` ne peut plus éteindre le
    // routeur (le bouton Disconnect d'upstream l'écrivait, constat 2026-09-19).
