...
**Nemo** is the client of **[CoeOS](https://github.com/Odyssai-eu/coeos-box)**, the smart
router of the OdyssAI stack. Install the app, point it at a CoeOS box or at an
...
4. Nemo finds its backend on its own: an `OdyssAI-X` engine (port 8000) or a CoeOS box
   (port 4600) on the LAN, a provisioned pairing file (`~/.nemo/pairing.json`), or the
   `CODEOS_ENGINE_URL` environment variable. Nothing is hardcoded in the binary.
...
| **CoeOS** | [Odyssai-eu/coeos-box](https://github.com/Odyssai-eu/coeos-box) | The smart router: competence axes, provider table, local-vs-cloud policy. |
| **Nemo** | this repo | The client. |
...
Nemo is a fork of **[opencode](https://github.com/sst/opencode)** (MIT). The agent runtime,
the desktop shell and the tool plumbing are theirs; the modes, agents, routing, guardian
and memory are ours. Engine internals stay close to upstream so improvements merge back.
